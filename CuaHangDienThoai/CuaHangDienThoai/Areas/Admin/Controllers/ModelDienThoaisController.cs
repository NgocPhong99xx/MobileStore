...
namespace CuaHangDienThoai.Areas.Admin.Controllers
{
    ...
    public class ModelDienThoaisController : Controller
    {
        ...
        // Xóa model điện thoại
        public async Task<IActionResult> Delete(int? maModel)
        {
            var Role = HttpContext.Session.GetString(CommonAdmin.ROLE_SESSION);
            if (Role != null)
            {
                ...
                if (maModel == null)
                    return NotFound();
                ModelDienThoaiVM.ModelDienThoais = await _mb.ModelDienThoai.Include(m => m.DienThoais).SingleOrDefaultAsync(m => m.MaModel == maModel);
                if (ModelDienThoaiVM.ModelDienThoais == null)
                    return NotFound();
                return View(ModelDienThoaiVM);
            }
        ...
        [ValidateAntiForgeryToken]
        [Obsolete]
        public async Task<IActionResult> DeleteConfirmed(int maModel)
        {
            string webPathRood = _hostingEnvironment.WebRootPath;
            ModelDienThoai modeldienThoai = await _mb.ModelDienThoai.FindAsync(maModel);
            if (ModelState.IsValid)
            {
                var uploads = Path.Combine(webPathRood, SD.ImageFolderModelDienThoai);
                var extention = Path.GetExtension(modeldienThoai.Hinh);
                if (System.IO.File.Exists(Path.Combine(uploads, modeldienThoai.MaModel + extention)))
                    System.IO.File.Delete(Path.Combine(uploads, modeldienThoai.MaModel + extention));
                _mb.Remove(modeldienThoai);
                await _mb.SaveChangesAsync();
                return RedirectToAction(nameof(Index));
            }
            return NotFound();
